...
export class ScheduleAssignComponent implements OnInit {
  ...
  toggleItem(name: string) {
    if (this.exists(name)) {
      this.selected = this.selected.filter(item => item !== name); // zelfde als splice
    } else {
      this.selected = [...this.selected, name]; // zelfde als push
    }
  }
  ...
  updateAssign() {
    this.update.emit({
      [this.section.type]: this.selected // Kan workouts of meals zijn
    });
  }
